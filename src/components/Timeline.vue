	...
	<section class="timeline">
		<div v-if="hasItems" class="wrapper-timeline">
			...
			>
				...
				<TimelineItem :item-timeline="timelineContent" :date-locale="dateLocale"/>
			</div>
		</div>
...
export default {
	...
	methods: {
		...
		hasYear(dataTimeline) {
			return (
				dataTimeline.hasOwnProperty("from") &&
				dataTimeline.from !== undefined
			);
		},
		...
		orderItems(items, typeOrder) {
			...
			return timeItemsOrdered
				.map(timeItem => itemsGrouped[timeItem])
				.flat();
		}
	}
};
